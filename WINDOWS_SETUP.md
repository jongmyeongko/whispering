...
3. (선택) **Auto-stop(min)** 에서 자동 종료 시간을 지정 (10분 단위, 최대 120분 / `0` = 사용 안 함)
4. **Start** 클릭 → 왼쪽 패널에 전사, 오른쪽 패널에 번역이 실시간 표시
5. **Stop** 클릭으로 종료

### 자동 종료 예약 (Auto-stop)

- **Auto-stop(min)** 값을 10~120(10분 단위)으로 지정하고 **Start** 를 누르면, 해당 시간이 지난 뒤 **자동으로 Stop** 됩니다.
- 실행 중에는 **Stop 버튼에 남은 시간이 카운트다운**으로 표시됩니다. (예: `Stop (0:59:58)`)
